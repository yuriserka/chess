...
export class Pawn extends Piece {
  ...
  private canMoveForward(board: Board, direction: number) {
  ...
  private getDoubleForwardMove(board: Board): Move | null {
    ...
    if (isWhiteInitial || isBlackInitial) {
      const direction = this.getDirection();
      const { canMove, newPosition } = this.canMoveForward(
        board,
        2 * direction
      );
      if (canMove) {
  ...
  private canMoveDiagonally(board: Board, direction: number) {
    ...
    return {
      canMoveRight:
        board.isValidPosition(newPositionRight) &&
        board.getPiece(newPositionRight) !== null,
      newPositionRight: newPositionRight,
      canMoveLeft:
        board.isValidPosition(newPositionLeft) &&
        board.getPiece(newPositionLeft) !== null,
  ...
  private getCaptureMove(board: Board): Move[] {
    const direction = this.getDirection();
    const { canMoveRight, newPositionRight, canMoveLeft, newPositionLeft } =
      this.canMoveDiagonally(board, direction);
